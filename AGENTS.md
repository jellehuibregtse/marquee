...
- `cmd/marquee/` — the CLI. `main.go` routes the subcommands and wires wrapper mode end to end; `attach.go` is the attach subcommand (pure proxy, no child); `options.go` parses and validates flags; `convention.go` resolves the per-repo `.marquee/` conventions (today the `hook` script that becomes `--switch-hook`); `config.go` turns `.marquee/config` into flag words that are parsed ahead of the command line; `pidfile.go` records the child's process-group id per listen address; `diagnose.go` turns `net.Listen` failures into friendly diagnostics; `log.go` is the `--quiet`-aware logger.
- `cmd/marquee/`, client side — the `status` and `switch` subcommands, which are plain HTTP clients of `/__marquee/status` and `/__marquee/switch` and add no guard of their own. `session.go` is the session file (listen address, switch token, pid) that lets a second process find a running marquee: written beside the pidfile once the listener answers, removed on shutdown, swept of stale entries by `liveSessions`. `control.go` decides which marquee a subcommand means (an explicit `--listen`, else the only one running, else the one whose worktree list contains the working directory) and holds the two calls. `statuscmd.go` and `switchcmd.go` are the subcommands themselves, including the client-side slug check that exists only so a typo gets the real target list back.
...
- `internal/gitinfo/` — shells out to git. `parseWorktrees` parses `git worktree list --porcelain` into `Worktree{Slug, Path, Branch}`; the first listed worktree is the main one. `orderByRecency` then reorders that list for the bar's menu: main stays pinned first, the rest follow `for-each-ref --sort=-committerdate`, and a worktree with no dated branch (detached HEAD) sorts last. `filter.go` holds `WorktreeFilter`, the `--worktree-glob` target set applied to both the status payload and `Orchestrator.Prepare` (the main worktree always survives it).
